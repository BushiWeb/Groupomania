...
export default function PostHeader({ onLike, onMoreActions }) {
    const { authorEmail, emailId, date, dateId } = useContext(postContext);
    ...
    return (
        <header className={style.header}>
            <div className={style.postInfo}>
                <address className={style.author} id={emailId}>
                    {authorEmail}
                </address>
                <time className={style.date} dateTime={date} id={dateId}>
                    {formatedDate}
                </time>
            </div>

            <PostLike onClick={onLike} />

            <PostActions onClick={onMoreActions} />
        </header>
    );
}
