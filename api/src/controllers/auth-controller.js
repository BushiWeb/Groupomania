 ...
 * Sends a message to the client with status 200 and the authentication tokens if the request is successful, or calls
 *  the error handler middleware if an error occurs.
...
export async function loginController(req, res, next) {
    ...
    try {
        ...
        if (error instanceof NotFoundError) {
            normalizedError = new UnauthorizedError({ message: 'The user corresponding the the email can\'t be found' }, error, {});
        }
 ...
 * Sends a message to the client with status 204 if the logout is successful, or calls the error handler middleware if
 *  an error occurs.
 * @param {Express.Request} req - Express request object.
 * @param {Express.Response} res - Express response object.
 * @param next - Next middleware to execute.
...
export async function getAccessTokenController(req, res, next) {
    ...
    try {
        ...
        const refreshToken = await createRefreshToken(
            ...
            currentRefreshToken.roleId,
            currentRefreshToken.exp
        );
