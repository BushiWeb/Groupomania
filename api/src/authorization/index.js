...
/**
 * Object containing all access control rules and the PEP (Policy Execution Point).
 * Serves the rules that applies to a request.
 * @namespace
...
const accessControlRules = {
     ...
     * @returns {Condition|boolean} Returns :
     *      - An AST condition regrouping all conditions to meet;
     *      - true if no conditions are found;
     *      - false if no rules are matching.
     */
     ...
     * @param {Object} base - Base object, contains informations about the entity. These informations will be used to
     *  fetch the entry from the database if needed. Thus, they need to have only properties defined in the model, and
     *  to avoid unpredicable results, they need to be able to identify a single entry. Providing the primary key is a
     *  safe way to go about it.
     * @returns {Proxy} Returns a proxy of the base object, intercepting all property access to make sure they are
     *  present. If event after fetching the entity informations the property can't be found, the proxy throws a
     *  InternalServerError. If no entity matches the base, the proxy throws a NotFoundError.
     * @throws {InternalServerError} Throws if the required property is not defined in the Model.
     * @throws {NotFoundError} Throws if the entry corresponding to the base data doesn't exist, or if multiple entries
     *  are returned.
     */
    PIP: function (modelName, base) {
        ...
        const handler = {
            get: async function (target, prop) {
                ...
                const targetEntry = await Model.findAll({ where: base });
